...
@HOOKS_REG.register_module()
class OldCkptLoadHook(BaseHook):
    """
    A custom hook for incremental learning, load ckpt to old network before training
    """
    ...
    def load_ckpt(self, runner):
        ...
        runner.logger.write("Loading old ckpt...")
        device = get_device()
        ckpt = torch.load(self.path, map_location=device)
    ...
    def load_net_state_dict(self, runner, state_dict):
        runner.net_dict['old'].load_state_dict(state_dict['main'], strict = self.strict)

@HOOKS_REG.register_module()
class CodeSaveHook(BaseHook):
    """
    Args:
        path: path to save code
        method: project name
    Support 2 level dir code to save
    """
    master_only = True
    ...
    def save_code(self, runner):
        src_path = os.path.join(self.path, 'src')
        os.makedirs(os.path.join(src_path), exist_ok=True)
        src_to_save = glob.glob('*.py')
        for src in src_to_save:
            dst_file = os.path.join(self.path, 'src', os.path.basename(src))
            shutil.copyfile(src, dst_file)
        src_dir_list = ['datasets', 'hooks', 'models', 'modules', 'networks', 
                        'runners', 'utils']
        for ds in src_dir_list:
            src_to_save = glob.glob(ds + '/*.py')
            src_to_save += glob.glob(ds + '/*/*.py')
            for src in src_to_save:
                dst_path = os.path.join(src_path, os.path.dirname(src))
                os.makedirs(dst_path, exist_ok=True)
                dst_file = os.path.join(dst_path, os.path.basename(src))
                shutil.copyfile(src, dst_file)
