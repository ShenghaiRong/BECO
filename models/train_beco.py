...
from utils.registers import MODELS_REG
...
from utils.visual.simseg import vis_data, vis_mask


@MODELS_REG.register_module()
class BECO(BaseSimSeg):
    ...
    def __init__(self, config, nets_dict) -> None:
        ...
        self.init_loss()
        self.ignore_bg = config.model.settings.get('ignore_bg', False)
        self.mix_aug = config.model.settings.get('mix_aug', False)
        self.mix_prob = config.model.settings.get('mix_prob', 1)
        self.warm_up = config.model.settings.get('warm_up', 0)
        self.bdry_size = config.model.settings.get('bdry_size', 1)
        self.bdry_whb = config.model.settings.get('bdry_whb', 0.)
        self.bdry_whi = config.model.settings.get('bdry_whi', 0.)
        self.bdry_wlb = config.model.settings.get('bdry_wlb', 0.)
        self.bdry_wli = config.model.settings.get('bdry_wli', 0.)
        self.T = config.model.settings.get('highres_t', 0)
        self.save_logits = config.model.settings.get('save_logits', False)
        self.max_epoch = config.runner.settings.max_epoch
        self.test_msc = config.model.settings.get('test_msc', False)
    ...
    def forward_cat_test(self, images: Tensor):
        out = {}
        out1 = self.nets['student'](images)
        out2 = self.nets['teacher'](images)
